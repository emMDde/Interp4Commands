    ...
    */
    class AbstractMobileObj {
     public:
        ...
        */
       virtual const Vector3D & GetTrans_m() const = 0;
       /*!
        ...
        *         że współrzędne wyrażone są w metrach.
        */
       virtual void SetTrans_m(const Vector3D &rPos) = 0;
        ...
        */
        ...
        virtual void SetShift(const Vector3D& shift) = 0;

        virtual const VectorRGB& GetColor() const = 0;

        virtual void SetColor(const VectorRGB& color) = 0;

        virtual void LockObj() = 0;

        virtual void UnlockObj() = 0;

    };

inline
std::ostream& operator << (std::ostream &OStrm, const AbstractMobileObj &rObj)
{
    OStrm << "Name=" << rObj.GetName() << " ";
    Vector3D Rotations;
    Rotations[0] = rObj.GetAng_Roll_deg();
    Rotations[1] = rObj.GetAng_Pitch_deg();
    Rotations[2] = rObj.GetAng_Yaw_deg();
    
    OStrm << "RotXYZ_deg=" << Rotations << " ";
    OStrm << "Scale=" << rObj.GetScale() << " ";
    OStrm << "Shift=" << rObj.GetShift() << " ";
    OStrm << "Trans_m=" << rObj.GetTrans_m() << " "; 
    OStrm << "RGB=" << rObj.GetColor();
